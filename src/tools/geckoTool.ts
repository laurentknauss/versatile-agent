...
import { z } from 'zod';
import { CoinMarketData, CoinPrice } from './types/gecko';
...
/** Hard deadline for a CoinGecko request. */
const FETCH_TIMEOUT_MS = Number(process.env.COINGECKO_FETCH_TIMEOUT_MS ?? 15_000);

/**
 * CoinGecko call with a hard deadline.
 * node-fetch reports an expired AbortSignal as AbortError, not TimeoutError.
 */
async function fetchCoinGecko(url: string, headers: Record<string, string>) {
  try {
    ...
    if (expired) {
      throw new Error(`CoinGecko did not respond within ${FETCH_TIMEOUT_MS} ms.`);
    }
...
const tracedCoinGeckoPriceTool = async ({
  coinIds,
  vsCurrencies,
  includeMarketCap,
  includeVolume,
  ...
  const response = await fetchCoinGecko(url, headers);
...
export const coinGeckoPriceTool = tool(tracedCoinGeckoPriceTool, {
  name: 'coinGeckoPrice',
  description:
    "Get current cryptocurrency prices from CoinGecko. Use coin IDs like 'bitcoin', 'ethereum', 'solana', etc.",
  schema: z.object({
    coinIds: z
      .string()
      .describe("Comma-separated list of coin IDs (e.g., 'bitcoin,ethereum,solana')"),
    vsCurrencies: z
      .string()
      .optional()
      .describe("Comma-separated list of currencies (default: 'usd')"),
    ...
    includePriceChange: z
      .boolean()
      .optional()
      .describe('Include 24h price change data (default: true)'),
  }),
});

// Tool 2 : Get market data for top crypto-currencies
const tracedCoinGeckoMarketTool = async ({
  vsCurrency,
  perPage,
  page,
  category,
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // Corps d'erreur non JSON — on garde le statut HTTP seul.
    }
    if (category) {
...
export const coinGeckoMarketTool = tool(tracedCoinGeckoMarketTool, {
  ...
  description:
    'Get market data for top crypto-currencies by market cap from CoinGecko. Supports filtering by category, coin IDs, and multi-timeframe price changes.',
  schema: z.object({
    ...
    category: z
      ...
      .optional()
      .describe(
        "CoinGecko category slug. Common values: 'non-fungible-tokens-nft' (NFT), 'decentralized-finance-defi' (DeFi), 'gaming' (GameFi), 'meme-tokens' (Meme), 'ai-agents', 'ai-applications'. Use /coins/categories/list to discover all slugs."
      ),
    ids: z
      .string()
      .optional()
      .describe(
        "Comma-separated list of coin IDs to filter (e.g., 'bitcoin,ethereum,solana'). Overrides category filter."
      ),
    priceChangePercentage: z
      .string()
      .optional()
      .describe(
        "Comma-separated timeframes for price change: '1h,24h,7d,14d,30d,60d,200d,1y' (default: none)"
      ),
    precision: z
      .string()
      .optional()
      .describe("Decimal precision: 'full' or '0'-'18' (default: API default)"),
  }),
});
