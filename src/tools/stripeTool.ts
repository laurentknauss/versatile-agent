...
// SDK defaults: 80 000 ms per attempt with 2 retries (~4 min worst case) — far too
// long for a chat turn. Same env-override shape as the fetch tools (geckoTool, movieTool, weatherTool).
const STRIPE_TIMEOUT_MS = Number(process.env.STRIPE_TIMEOUT_MS ?? 10_000);
const STRIPE_MAX_RETRIES = Number(process.env.STRIPE_MAX_RETRIES ?? 1);
...
export const stripeAccountInfoTool = tool(tracedStripeAccountInfoTool, {
  ...
  description:
    'Get basic information about your Stripe account including balance, status, and capabilities',
...
export const stripeCustomersListTool = tool(tracedStripeCustomersListTool, {
  ...
  schema: z.object({
    limit: z
      ...
      .max(100)
      .optional()
      .describe('Number of customers to retrieve (max 100, default 10)'),
  }),
});
 ...
 */
const tracedStripeCreateCustomerTool = async ({
  email,
  name,
  description,
}: {
...
export const stripeCreateCustomerTool = tool(tracedStripeCreateCustomerTool, {
  ...
  schema: z.object({
    ...
    description: z
      .string()
      .optional()
      .describe('Description or notes about the customer (optional)'),
  }),
});
...
export const stripeRecentEventsTool = tool(tracedStripeRecentEventsTool, {
  ...
  schema: z.object({
    limit: z
      ...
      .max(20)
      .optional()
      .describe('Number of recent events to retrieve (max 20, default 5)'),
  }),
});
